<template>
    <div class="second_level_page set_pay_pwd_page">
        <form class="setPayPwdForm">
            <section class="input_container">
                <input type="text" placeholder="请输入手机号" readonly="readonly" v-model="phoneNumber" @input="inputPhone">
            </section>
            <section class="input_container">
...
<script>
    ...
    export default {
        ...
        created(){
            ...
            this.phoneNumber = this.userInfo.tel;
            this.inputPhone();
        },
        methods: {
            //判断输入的电话号码
            ...
            async nextButton(){
                ...
                if (res.resultCode==0) {
                    ...
                    let telInfoStr=JSON.stringify(telInfo);
                    localStorage.setItem("telInfo", telInfoStr);

                    if(this.$route.meta && this.$route.meta.type == 'forget') {
                        this.$router.replace({ name: 'modifyPayPwd', params: { phoneNumber: this.phoneNumber }})
                    } else{
                        this.$router.replace({ name: 'confirmPayPwd', params: { phoneNumber: this.phoneNumber }})
                    }         
                } 
