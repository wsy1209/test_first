 ...
 * 图片路径转换为小图路径'*_small.*'
 */
export const imgToSamll = (s) => {
    let imgBaseUrl = 'http://192.168.10.67';
    let defaultImg=imgBaseUrl+'/sys/default/goodsDefault.png';
    if(s==null||s==""){
        return '';//可以指定一个默认图片地址。
    }    
    if(s=='http://192.168.10.67/'){
        return defaultImg;
    }
    let b = s.substring(0,s.lastIndexOf('.'))+"_small";
    let a = s.substring(s.lastIndexOf('.'));
    s = b+a;
    // 图片路径不对或者格式不对，可以指定一个默认图片地址。
